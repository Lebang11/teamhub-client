import { Link } from "react-router-dom";

const Gamer = (props) => {
    return ( 
        <div>
            <Link to={`/challenge/${props.gamer._id}`} className="problem-link">
                <div className="text-center">
                    
                    <h1 className="display-5 author">
                        {props.gamer.name}
                    </h1>

                {props.gamer.steamName !== '' && 
                
                    <p className="text-info"> <i class="fa-brands fa-steam text-light me-2"></i><span className="text-light">{props.gamer.steamName}</span></p>
                }
                {props.gamer.psnName !== '' &&
 <p className="text-info"><i class="fa-brands fa-playstation text-light me-2"></i>
 <span className="text-light">{props.gamer.psnName}</span></p>
                }
                {props.gamer.xboxName !== '' && 
                
                    <p className="text-info"> <i class="fa-brands fa-xbox text-light me-2"></i><span className="text-light">{props.gamer.xboxName}</span></p>
                }
                {props.gamer.epicName !== '' &&
                <p className="text-info">                <img  src="data:image/png;base64,"/>
                <span className="text-light">{props.gamer.epicName}</span></p>
                }
                {props.gamer.eaName !== '' &&
                <p className="text-info"><img style={{
                    "width": "4vw"
                }} src="data:image/png;base64,"/>
                
                     <span className="text-light">{props.gamer.eaName}</span></p>
                }
                   
                    
                    
                    
                    <h4>My games: </h4>
                    <p className="text-light">{props.gamer.games}</p>
                    <div className="blog-date">
                        <div >{props.gamer.date}</div>
                    </div>
                </div> 
            </Link>
        </div>
     );
}
 
export default Gamer;